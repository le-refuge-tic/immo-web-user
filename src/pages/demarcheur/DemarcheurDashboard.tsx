...
type Tab = 'tableau' | 'biens' | 'reservations' | 'profil'
          ...
          return (
            <div key={v.id || i} className="bg-white rounded-2xl shadow-sm mb-3 p-4">
              ...
              <div className="flex gap-2">
                ...
                {v.statut === 'confirmee' && (
                  <button onClick={() => confirmer(v.id)} className="flex-1 py-2 rounded-xl text-white text-xs font-bold" style={{ background: PURPLE }}>Marquer effectuée</button>
                )}
              </div>
              {cpId === v.id && (
